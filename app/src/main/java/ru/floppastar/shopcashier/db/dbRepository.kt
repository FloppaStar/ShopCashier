...
class dbRepository(private val dbHelper: DatabaseHelper) {
    ...
    fun deleteGoodsType(goodsTypeId: Int) {
        val db = dbHelper.writableDatabase
        db.delete(
            DatabaseHelper.TABLE_GOODS_TYPE,
    ...
    // Получение или создание saleId для текущей продажи
    ...
    fun removeGoodsFromCart(goodId: Int, saleId: Int) {
        ...
        db.beginTransaction()
        try {
            // Получить ID первой записи, которая соответствует критериям
            val cursor = db.rawQuery(
    ...
    fun completeSale(saleId: Int): Boolean {
        ...
        db.beginTransaction()
        try {
            // 1. Обновление количества товаров на складе
            ...
            // 2. Обновление записи SaleInformation
            val contentValues = ContentValues().apply {
            ...
            // 3. Создание новой записи SaleInformation
